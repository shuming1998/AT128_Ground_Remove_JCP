...
ProjectionJpc::ProjectionJpc() : nh("~") {
  pts_sub = nh.subscribe<sensor_msgs::PointCloud2>(
      pts_topic, 10, &ProjectionJpc::runProject, this);
...
void ProjectionJpc::mapMake() {
  ...
  for (const auto pt : pts_in->points) {
    ...
    if (pt.ring != 0) {
      ...
      region_index =
          col_i * radial_num + radial_index; // radial_num为每条轴线总环数
      ...
      region_image.at<uchar>(row_i, col_i) =
          radial_index;           // region_中存储该点所在环数
...
void ProjectionJpc::RECM() {
  ...
  for (int i = 0; i < region_minz.size(); ++i) {
    if (i % radial_num == 0) {
      ...
      region_minz[i] =
          min(region_minz[i],
              region_minz[i - 1] + (float)(delta_R * tan(sigma / RAD)));
    }
  }
...
void ProjectionJpc::JPC() {
  ...
  for (int row = 0; row < scan_num; ++row) {
    for (int col = 0; col < horizon_pts_num; ++col) {
      if (range_image.at<cv::Vec3b>(row, col) == cv::Vec3b(0, 255, 255) &&
          cloud_index[col * scan_num + row] != -1) {
        Index pt_judge;
        pt_judge.x = col;
        pt_judge.y = row;
  ...
  while (!pts_need_rejudge.empty()) {
    ...
    Eigen::VectorXf D(24);
    int pt_id = pt_judge.x * scan_num +
                pt_judge.y; // pt_judge.x = col、 pt_judge.y = row
    int mask[24];
    float sumD(0), diff(0);
    Point p1, p2;
    for (int i = 0; i < 24; ++i) {
      ...
      if (neighbor_xi < 0 || neighbor_xi >= horizon_pts_num ||
          neighbor_yi < 0 || neighbor_yi >= scan_num ||
          cloud_index[neighbori_id] == -1) {
        D(i) == 0;
        sumD += D(i);
        mask[i] = -1;
        continue;
      ...
      if (range_image.at<cv::Vec3b>(neighbor_yi, neighbor_xi) ==
          cv::Vec3b(255, 0, 0)) // range内低置信度的点
      {
        mask[i] = 2;
      } else if (range_image.at<cv::Vec3b>(neighbor_yi, neighbor_xi) ==
                 cv::Vec3b(0, 255, 0)) //接地点
      {
        mask[i] = 1;
      } else if (range_image.at<cv::Vec3b>(neighbor_yi, neighbor_xi) ==
                 cv::Vec3b(0, 0, 255)) //障碍点和range之外的低置信度的点
      {
        mask[i] = 0;
      }
    }
...
//点云格式转换
void ProjectionJpc::ptsCopy(
    const sensor_msgs::PointCloud2::ConstPtr &origin_pts) {
...
void ProjectionJpc::runProject(
    const sensor_msgs::PointCloud2ConstPtr &pts_msg) {
